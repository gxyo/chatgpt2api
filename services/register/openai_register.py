...
def _is_cloudflare_challenge(resp) -> bool:
    ...
    try:
        ...
        status_code = 0
    if raw_status is not None and status_code not in (403, 503):
        return False
...
def create_session(proxy: str = "") -> Any:
    ...
    if any(key in kwargs for key in ("impersonate", "proxy")):
...
def _apply_clearance_to_session(session: Any, bundle: ClearanceBundle | None) -> None:
    if bundle is None:
        return
